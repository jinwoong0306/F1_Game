...
#### 3.1 체크포인트 시스템
- [ ] `loadCheckpointsFromMap()` 메서드 이식
- [ ] "checkpoints" 레이어에서 사각형 로드
- [ ] index 속성 기반 순차 검증 로직
- [ ] 체크포인트 통과 시각 피드백

#### 3.2 출발선 및 랩 카운팅
- [ ] `loadStartLineFromMap()` 메서드 이식
- [ ] "start_line" 레이어 처리
- [ ] 랩 완료 감지 (모든 체크포인트 + 출발선 통과)
- [ ] 랩타임 기록 및 베스트 랩 업데이트

#### 3.3 Grass 영역 (오프트랙)
- [ ] `createGrassZone()` 메서드 이식
- [ ] "Grass" 레이어에서 폴리곤 추출
- [ ] 차량이 Grass 진입 시 속도 페널티 적용
- [ ] 오프트랙 시각 효과 (HUD 경고)

#### 3.4 피트스톱 영역
- [ ] `loadPitAreasFromMap()` 메서드 이식
- [ ] "pit_entry", "pit_service", "pit_exit" 레이어 로드
- [ ] 피트 진입 → 서비스 → 퇴장 상태 전환
- [ ] 피트레인 속도 제한 (80 km/h)
- [ ] 피트스톱 미니게임 트리거

#### 3.5 충돌 감지 개선
- [ ] Box2D ContactListener 리팩토링
- [ ] 벽 충돌 시 차량 내구도 감소
- [ ] 충돌 강도 계산 (속도 기반)
- [ ] 충돌 효과음 및 시각 효과
...
- [ ] america.tmx 맵 테스트
- [ ] japan.tmx 맵 테스트
- [ ] f1_racing_map.tmx 맵 테스트
- [ ] 모든 맵에서 체크포인트/피트 동작 확인
